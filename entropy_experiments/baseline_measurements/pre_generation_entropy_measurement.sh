...
if [[ $copy_script == "y" ]]; then
    ...
    scp -i "$private_ssh_key" "$ENTROPY_MEASUREMENT_SCRIPT_PATH" "$remote_ssh_user@$remote_ssh_host:~/pre_generation_entropy_measurement.py"
else
...
if [[ $run_inference == "y" ]]; then
    ...
    ssh -i "$private_ssh_key" "$remote_ssh_user@$remote_ssh_host" "nohup python3 ~/pre_generation_entropy_measurement.py --model_variant $model_variant > ${model_variant}_entropy_measurement_output.log 2>&1 &" &
else
